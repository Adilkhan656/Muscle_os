...
class ProfileActivity : AppCompatActivity() {
    ...
    private val dataSource by lazy {
        FirestoreUserProfileDataSource(FirebaseFirestore.getInstance(), auth)
    }
    ...
    private fun loadProfile() {
        ...
        lifecycleScope.launch {
            dataSource.fetchUserProfile(user.uid)
                .onSuccess { profile ->
                    if (profile == null) {
                ...
                .onFailure { error ->
                    Toast.makeText(this@ProfileActivity, error.message ?: "Unable to load profile.", Toast.LENGTH_SHORT).show()
                }
        }
    }
    ...
    private fun renderTags(container: LinearLayout, values: List<String>) {
        ...
        if (values.isEmpty()) {
            val empty = TextView(this).apply {
                ...
                setTextColor(getColor(R.color.premium_muted))
                textSize = 13f
            }
    ...
    private fun LinearLayout.flexWrap() { /* Chips stay compact; parent scroll handles overflow. */ }
