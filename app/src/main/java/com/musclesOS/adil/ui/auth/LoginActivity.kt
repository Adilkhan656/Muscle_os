...
class LoginActivity : AppCompatActivity() {
    ...
    private val viewModel by lazy { AuthViewModel(repository) }
        ...
        binding.btnFacebookLogin.setOnClickListener {
            requireInternet {
                facebookAuthManager.login(
                    onSuccess = { token -> viewModel.facebookLogin(token) },
                    onError = {
                        Toast.makeText(
                            this,
                            it.message,
    ...
    override fun onActivityResult(
        ...
        facebookAuthManager.getCallbackManager().onActivityResult(
            requestCode,
            resultCode,
            data
        )
    ...
    private fun handleGoogleResult(result: GetCredentialResponse) {
        ...
        when {
            credential is CustomCredential &&
                ...
                viewModel.signIn(googleCredential.idToken)
            }

            credential is PasswordCredential -> {
                viewModel.login(credential.id, credential.password)
            }
    ...
    private fun openMain() {
        lifecycleScope.launch {
            ...
            val intent = when (destination) {
                ...
                else -> Intent(this@LoginActivity, OnboardingActivity::class.java)
            }
